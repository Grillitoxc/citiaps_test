  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <header class="border-b bg-white">
      <div class="mx-auto max-w-6xl px-4 py-4 flex items-center justify-between">
        <h1 class="text-2xl font-bold">Métricas</h1>
        <button class="rounded border px-3 py-2 text-sm" @click="$router.push('/posts')">
    ...
    <div class="mx-auto max-w-6xl px-4 py-6">
      ...
      <div class="grid gap-4 grid-cols-1 md:grid-cols-2 xl:grid-cols-4">
        <div class="rounded-lg border bg-white p-4">
          <div class="text-sm font-medium text-gray-700 flex items-center justify-between">
            Total Posts
            <svg class="w-4 h-4 text-gray-400" viewBox="0 0 24 24" fill="none"><path d="M6 4h9a2 2 0 0 1 2 2v1h1a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2V4Z" stroke="currentColor" stroke-width="1.5"/></svg>
          </div>
        ...
        <div class="rounded-lg border bg-white p-4">
          <div class="text-sm font-medium text-gray-700 flex items-center justify-between">
            Publicados
            <svg class="w-4 h-4 text-gray-400" viewBox="0 0 24 24" fill="none"><path d="M12 5c3.866 0 7 3.582 7 8s-3.134 8-7 8-7-3.582-7-8 3.134-8 7-8Z" stroke="currentColor" stroke-width="1.5"/><circle cx="12" cy="13" r="3" stroke="currentColor" stroke-width="1.5"/></svg>
          </div>
        ...
        <div class="rounded-lg border bg-white p-4">
          <div class="text-sm font-medium text-gray-700 flex items-center justify-between">
            Borradores
            <svg class="w-4 h-4 text-gray-400" viewBox="0 0 24 24" fill="none"><path d="M7 3h10v4H7zM5 7h14v14H5z" stroke="currentColor" stroke-width="1.5"/></svg>
          </div>
        ...
        <div class="rounded-lg border bg-white p-4">
          <div class="text-sm font-medium text-gray-700 flex items-center justify-between">
            Tags Únicos
            <svg class="w-4 h-4 text-gray-400" viewBox="0 0 24 24" fill="none"><path d="M7 7h.01M3 12l7.586-7.586A2 2 0 0 1 12 4h8v8a2 2 0 0 1-.586 1.414L12 21 3 12Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
          </div>
              ...
              <li v-for="(t, i) in topTags" :key="t.tag" class="flex items-center gap-3">
                ...
                <div class="flex-1">
                  ...
                  <svg :width="'100%'" height="8" class="mt-1">
                    <rect x="0" y="0" :width="(t.count / (topTags[0]?.count || 1)) * 100 + '%'" height="8" rx="4" class="fill-gray-800" />
                    <rect :x="(t.count / (topTags[0]?.count || 1)) * 100 + '%'" y="0" :width="(100 - (t.count / (topTags[0]?.count || 1)) * 100) + '%'" height="8" rx="4" class="fill-gray-200" />
                  </svg>
                </div>
              </li>
            </ul>
        ...
        <div class="rounded-lg border bg-white p-4">
          ...
          <div class="mt-2">
            ...
            <div class="mt-2 flex justify-between text-sm text-gray-700">
              <div><span class="inline-block w-3 h-3 bg-black mr-2"></span>Publicados ({{ totals.published }})</div>
              <div><span class="inline-block w-3 h-3 bg-gray-300 mr-2"></span>Borradores ({{ totals.drafts }})</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="grid gap-4 grid-cols-1 xl:grid-cols-2 mt-6">
          ...
          <ul class="divide-y">
            <li v-for="(a, idx) in authors" :key="a.name" class="flex items-center justify-between py-3">
              <div class="flex items-center gap-3">
                <span class="w-8 h-8 rounded-full bg-gray-100 text-gray-700 flex items-center justify-center text-sm font-medium">
                  {{ idx + 1 }}
                </span>
        ...
        <!-- (No hacer Actividad Mensual) -->
        <div class="rounded-lg border bg-white p-4 opacity-60 select-none">
          <div class="text-base font-semibold">Actividad Mensual</div>
          <div class="text-sm text-gray-500">Posts creados en los últimos meses</div>
          <div class="text-sm text-gray-400 mt-6">[Este bloque se omite por requerimiento]</div>
        </div>
      </div>

      <!-- Todos los tags -->
      <div class="rounded-lg border bg-white p-4 mt-6">
        ...
        <div class="flex flex-wrap gap-2">
          <span v-for="t in allTags" :key="t.tag"
            ...
            <svg class="h-3.5 w-3.5 text-gray-500" viewBox="0 0 24 24" fill="none" aria-hidden="true">
              <path d="M7 7h.01M3 12l7.586-7.586a2 2 0 0 1 1.414-.586H20a1 1 0 0 1 1 1v8a2 2 0 0 1-.586 1.414L12 21 3 12z"
